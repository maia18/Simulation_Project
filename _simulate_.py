...
#       distance_ue_ap = sqrt( ( ( ( ue.position_ue - ap.position_ap[0] ) ** 2 ) ) + ( ( ( ue.position_ue[1] - ap.position_ap[1] ) ** 2 ) ) ) # Distance UE-AP
...
#         interference_ = 0
#         interference_ += ( ues.power *  ( ( distance_ues / ( simulate.do ) ** ( simulate.n ) ) ) ) # interference totally
...
class PointAcess: # Point Acess
  ...
  # class constructor
  def __init__(self, coverage_area, power_=0):
...
class Simulation: # Simulation
  ...
  def position_UE(self, ue: UserEquipments, ap:PointAcess, pos__ue=(0,0)): # Position UE
    ...
    radius = len(ap.coverage_area)//2  # Radius of the coverage area
